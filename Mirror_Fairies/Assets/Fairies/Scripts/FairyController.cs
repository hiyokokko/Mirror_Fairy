...
public class FairyController : MonoBehaviour
{
	...
	void Start()
	{
		...
		patternCount = new int[attackInfo.Length];
	}
	void Update()
	{
	...
	void Attack()
	{
		for (int pattern = 0; pattern < attackPattern.Length; pattern++)
		{
			if ((attackPattern[pattern] & (long)1 << attackCount) != 0)
			{
				...
				if (aim[pattern] || targetShiftPosX[pattern] || targetShiftPosY[pattern] || action[pattern])
				{
					try
					{
				...
				for (int infoNum = 0; infoNum < attackInfo[pattern].vector3Array[patternCount[pattern]].vector3.Length; infoNum++)
				{
					Vector3 info = attackInfo[pattern].vector3Array[patternCount[pattern]].vector3[infoNum];
					Vector2 burretPos = 
						new Vector2
						(
							info.x +
					...
					if (aim[pattern] || action[pattern])
					{
						burretInst.GetComponent<BurretController>().targetPos = targetPos + aimPos[pattern].vector2Array[patternCount[pattern]].vector2[infoNum];
					}
				}
				patternCount[pattern] = patternCount[pattern] < attackInfo[pattern].vector3Array.Length - 1 ? patternCount[pattern] + 1 : 0;
			}
		}
		if (attackCount < 47)
		{
